...
public class AsyncSearchManagementService extends AbstractLifecycleComponent implements LocalNodeMasterListener {
    ...
    public static final Setting<TimeValue> TASK_CANCELLATION_INTERVAL_SETTING =
            Setting.timeSetting("async_search.expired.task.cancellation_interval", TimeValue.timeValueMinutes(30),
                    TimeValue.timeValueMinutes(1), Setting.Property.NodeScope);
    public static final Setting<TimeValue> RESPONSE_CLEAN_UP_INTERVAL_SETTING =
            Setting.timeSetting("async_search.expired.response.cleanup_interval", TimeValue.timeValueMinutes(30),
                    TimeValue.timeValueMinutes(1), Setting.Property.NodeScope);
    ...
    class RunnableReaper implements Runnable {
        ...
        public void run() {
            try {
                ...
                transportService.sendRequest(randomNode, AsyncSearchCleanUpAction.NAME, new AsyncSearchCleanUpRequest(
                        threadPool.absoluteTimeInMillis()), new ActionListenerResponseHandler<AcknowledgedResponse>(
                        ActionListener.wrap((response) -> logger.debug("Successfully executed async search index clean up"),
